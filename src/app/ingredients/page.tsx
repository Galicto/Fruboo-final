...
export default function IngredientsPage() {
  return (
    <main className="bg-[#050505] min-h-screen text-white pb-32 overflow-hidden selection:bg-white/20">
      ...
      <section className="relative px-6 pt-48 pb-20 max-w-7xl mx-auto">
        <motion.div
      ...
      <section className="relative px-6 max-w-5xl mx-auto mt-10 space-y-8 md:space-y-12">
        {ingredients.map((item, i) => (
          <motion.div
            ...
            className={`relative p-8 md:p-12 rounded-[2.5rem] bg-gradient-to-r ${item.color} border border-white/5 backdrop-blur-3xl overflow-hidden group hover:border-white/10 transition-colors duration-500`}
          >
            <div className="absolute inset-0 bg-white/[0.02] opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <h2 className={`text-4xl md:text-6xl font-bold tracking-tight ${item.accent} drop-shadow-lg`}>
                {item.name}
              </h2>
              <p className="text-xl md:text-3xl text-white/70 font-light tracking-wider uppercase">
                {item.benefit}
              </p>
            </div>
          </motion.div>
      ...
      <section className="relative px-6 pt-40 pb-20 max-w-5xl mx-auto">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          ...
          transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] }}
        >
          <h2 className="text-5xl md:text-7xl font-bold tracking-tighter mb-6 drop-shadow-2xl text-center">
            Built around <span className="text-white/40 italic">your taste.</span>
          </h2>
          <p className="text-lg md:text-2xl text-white/60 font-light tracking-wide max-w-3xl mx-auto text-center mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {bases.map((base, i) => (
              <motion.div
                ...
                className={`p-8 rounded-3xl bg-black/40 border ${base.border} backdrop-blur-lg hover:bg-white/5 transition-all duration-300`}
              >
                <h3 className="text-2xl font-bold text-white tracking-tight mb-2">{base.name}</h3>
                <p className="text-white/60 font-light text-lg">{base.desc}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
