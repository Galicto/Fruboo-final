...
export default function BuyPage() {
  ...
  return (
    <main className="bg-[#050505] min-h-screen text-white flex flex-col selection:bg-white/20 overflow-hidden">
      ...
      <section className="relative flex-1 flex flex-col lg:flex-row items-center justify-center px-5 pt-20 md:pt-28 pb-16 md:pb-20 gap-8 md:gap-12 max-w-7xl mx-auto w-full">
        ...
        >
          ...
          <div className="relative w-full max-w-[280px] sm:max-w-sm aspect-[3/4] rounded-[2rem] bg-[#0c0c0c] flex items-end justify-center shadow-[inset_0_0_0_1px_rgba(255,255,255,0.06),0_0_100px_rgba(255,122,0,0.05)] overflow-hidden">
                ...
                <motion.img
                  ...
                  style={{
                    ...
                    clipPath: selected.size === "regular" 
                      ? "inset(14% 16% 0.5% 16% round 15% / 4%)"
                      : "inset(2% 16% 0.1% 16% round 12% / 3%)",
                  }}
                  animate={{ y: [0, -10, 0] }}
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 px-5 py-2 rounded-full bg-black/80 backdrop-blur-xl border border-white/10 z-20 shadow-2xl">
          ...
          {/* Size toggle */}
          <div className="mt-6 md:mt-8 flex items-center gap-2 md:gap-3 bg-white/5 p-1.5 rounded-full border border-white/10">
            {packs.map((p) => (
              <button
                key={p.id}
                onClick={() => setSelected(p)}
                className={`px-5 md:px-6 py-2 rounded-full text-xs md:text-sm font-bold tracking-widest uppercase transition-all duration-300 ${
                  selected.id === p.id
        ...
        >
          <p className="text-[11px] tracking-[0.5em] text-[#FF7A00] uppercase font-bold mb-4 md:mb-5">Order Now</p>
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-[0.9] mb-4 md:mb-6 text-transparent bg-clip-text bg-gradient-to-b from-white to-white/50">
            Secure your<br />freshness.
          </h1>
          <p className="text-base md:text-lg text-white/40 font-light mb-8 md:mb-12 leading-relaxed">
        ...
        {status === "success" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }}
              className="bg-[#0a0a0a] border border-white/10 p-8 md:p-12 text-center rounded-[2.5rem] md:rounded-[3rem] max-w-lg shadow-[0_0_100px_rgba(255,122,0,0.2)]">
              <div className="w-16 h-16 md:w-20 md:h-20 bg-[var(--color-accent-green)]/20 rounded-full flex items-center justify-center mx-auto mb-5 md:mb-6">
                <span className="text-3xl md:text-4xl">🎉</span>
              </div>
              <h2 className="text-3xl md:text-4xl font-black tracking-tighter text-white mb-3 md:mb-4">Order placed!</h2>
              <p className="text-base md:text-lg text-white/60 font-light mb-6 md:mb-8">Your fresh FRUBOO is being prepared.</p>
